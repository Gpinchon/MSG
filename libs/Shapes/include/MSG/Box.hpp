...
inline MSG::Box::Box(const glm::vec3& a_Center, const glm::vec3& a_HalfSize)
    : center(a_Center)
    , halfSize(a_HalfSize)
{
}

inline bool MSG::Box::IsInf() const { return glm::any(glm::isinf(halfSize)); }

inline glm::vec3 MSG::Box::Min() const { return center - halfSize; }

inline glm::vec3 MSG::Box::Max() const { return center + halfSize; }

inline void MSG::Box::SetMinMax(const glm::vec3& a_Min, const glm::vec3& a_Max)
{
...
inline std::array<glm::vec3, 8> MSG::Box::Points() const
{
    auto minP = Min();
    auto maxP = Max();
    return {
