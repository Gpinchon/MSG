...
void GenerateCubemapMipMaps(Texture& a_Texture)
{
    ...
    const auto baseLevel      = std::static_pointer_cast<Core::Cubemap>(a_Texture[0]);
    ...
    for (auto level = 1u; level <= mipNbr; level++) {
        ...
        auto mip       = std::make_shared<Core::Cubemap>(pixelDesc, levelSize.x, levelSize.y);
        mip->Allocate();
        ...
        for (auto side = 0u; side < 6; side++) {
            ...
            threadPool.PushCommand([&sideSrc, &sideDst] {
                sideSrc.Blit(sideDst, { 0u, 0u, 0u }, sideSrc.GetSize(), Core::ImageFilter::Bilinear);
            },
                false);
        }
        threadPool.Wait();
...
void Generate2DMipMaps(Texture& a_Texture)
{
    ...
    auto srcLevel             = std::static_pointer_cast<Core::Image2D>(a_Texture.front());
    a_Texture.resize(mipNbr);
    for (auto level = 1u; level < mipNbr; level++) {
        auto levelSize      = glm::max(baseSize / int(pow(2, level)), 1);
        auto mip            = std::make_shared<Core::Image2D>(pixelDesc, levelSize.x, levelSize.y);
        a_Texture.at(level) = mip;
        mip->Allocate();
        srcLevel->Blit(*mip, { 0u, 0u, 0u }, srcLevel->GetSize(), Core::ImageFilter::Bilinear);
        srcLevel = mip;
    }
}
...
auto Compress2D(Core::Image2D& a_Image, const uint8_t& a_Quality)
{
    auto inputSize                     = a_Image.GetSize();
    Core::BufferAccessor inputAccessor = a_Image.GetBufferAccessor();
    if (a_Image.GetPixelDescription().GetSizedFormat() != Pixel::SizedFormat::Uint8_NormalizedRGBA) {
        ...
        auto newImage = Core::Image2D(Pixel::SizedFormat::Uint8_NormalizedRGBA, inputSize.x, inputSize.y);
        newImage.Allocate();
    ...
    if (GetType() == TextureType::Texture2D) {
        ...
        for (auto& level : *this) {
            ...
            if (level->GetSize().x >= 4 && level->GetSize().y >= 4)
                result.emplace_back(Compress2D(*std::static_pointer_cast<Core::Image2D>(level), a_Quality));
        }
        *this = result;
    }
    SetCompressed(true);
